...
static void printHelp() {
  Serial.println("Serial commands: on | off | toggle | status | time | history | count | pending | sync | clearhistory | checkpoint | clearcheckpoint | recoverystatus | help");
}
...
void loop() {
  ...
  if (wasRecoveryActive && !recoveryActive) {
    if (networkIsConnected()) {
      firebasePublishLive();
